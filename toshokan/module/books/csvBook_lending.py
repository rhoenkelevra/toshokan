...
def csvBook_lending():
    ...
    cur.execute("select books.b_id, isbn, title, author, p_name, p_date, in_limit_date from books inner join log on books.b_id = log.b_id where books.d_flag = 1 and books.status = 0")
    rows = cur.fetchall()
